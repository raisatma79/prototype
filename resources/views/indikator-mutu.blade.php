        ...
        <div class="row g-4">
            ...
            <div class="col-md-4 col-lg-3">
                <div class="sticky-top" style="top: 100px;">
                    ...
                    <!-- Tampilan mobile -->
                    <div class="btn-group d-md-none" id="v-pills-tab-mobile" role="tablist" aria-label="Button group with nested dropdown">
                        <button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                            Pilih Tahun
                        </button>
                        <ul class="dropdown-menu">
                            @foreach($tabel_indikator_mutu as $tahun => $items)
                                <li><button class="dropdown-item {{ $loop->first ? 'active' : '' }}" id="v-pills-{{ $tahun }}-tab-mobile" data-bs-toggle="pill" data-bs-target="#v-pills-{{ $tahun }}" type="button" role="tab" aria-controls="v-pills-{{ $tahun }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">Tahun {{ $tahun }}</button></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
