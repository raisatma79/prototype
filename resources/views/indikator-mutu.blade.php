                ...
                <div class="sticky-top" style="top: 100px;">
                    ...
                    <div class="nav flex-column nav-pills me-3 space-y-2" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        @foreach($tabel_indikator_mutu as $tahun => $items)
                            <button class="btn btn-outline-info text-start fw-semibold fs-5 text-dark hover-primary my-1 {{ $loop->first ? 'active' : '' }}" id="v-pills-{{ $tahun }}-tab" data-toggle="pill" data-target="#v-pills-{{ $tahun }}" type="button" role="tab" aria-controls="v-pills-{{ $tahun }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">
